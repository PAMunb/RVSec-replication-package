...
clogger_deltaspyke <- sqldf("select distinct spec, class, classname, method from clogger_deltaspyke")
clogger_mcf <- sqldf("select distinct spec, class, classname, method from clogger_mcf")
clogger_meecrowave <- sqldf("select distinct spec, class, classname, method from clogger_meecrowave")
clogger_ds_kerberos_codec <- sqldf("select distinct spec, class, classname, method from clogger_ds_kerberos_codec")
clogger_tika <- sqldf("select distinct spec, class, classname, method from clogger_tika")
...
```{r}
gt_clogger <- sqldf("select distinct class, method, vulnerability 
                     from final_ds")
...
tp <- sqldf("select count(distinct (c.className || ' - ' || c.method))
             from clogger c, gt_clogger cgt 
             where c.className = cgt.class 
               and c.method = cgt.method
               and vulnerability =  'TRUE'")

fp <- sqldf("select count(distinct (c.className || ' - ' || c.method))
             from clogger c 
             where (c.className, c.method) not in 
               (select cgt.class, cgt.method 
                from gt_clogger cgt
                 where vulnerability =  'TRUE')")

fn <- sqldf("select count(distinct (cgt.class || ' - ' || cgt.method))
             from gt_clogger cgt
             where vulnerability =  'TRUE' 
               and (cgt.class, cgt.method) not in 
...
sqldf("select distinct (c.className || ' - ' || c.method)
             from clogger c 
             where (c.className, c.method) not in 
               (select cgt.class, cgt.method 
                from gt_clogger cgt
